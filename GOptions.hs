...
data OptionValue
  ...
  | FloatOpt Float
  | GnuSizeOpt GnuSize
  | GnuRangeOpt GnuRange
  deriving (Show)

data OptionEffect =
  OptionEffect (Options -> String -> [String] -> (Options, [String]))
...
appendFlag opts str val =
  addFlag (appendValue jFlag val) (removeFlag (getFlagOrPrefix str) opts)
  where
    jFlag =
      case (getFlag (getFlagOrPrefix str) opts) of
        (Just f) -> f
        Nothing ->
          (Option
             "<<ERROR>>"
...
getTargets opts =
  case (getFlag "--" opts) of
    (Just targets) ->
      case value targets of
        (ListOpt targetList) -> targetList
        _ -> []
    _ -> []

appendValue :: Option -> OptionValue -> Option
appendValue opt@(Option _ _ _ (ListOpt vals) _) (StringOpt val) =
  opt {value = ListOpt (vals ++ [val])}
...
removeFlag str (Options opts) =
  Options (filter (\x -> not (isFlag (getFlagOrPrefix str) x)) opts)

getFlag :: String -> Options -> Maybe Option
getFlag str (Options opts)
  | length matchOpts == 0 = Nothing
  | length matchOpts == 1 = Just (head matchOpts)
  | otherwise = Nothing
  where
    matchOpts = filter (\x -> (isFlag (str) x)) opts

isFlag :: String -> Option -> Bool
isFlag "" _ = False
isFlag str option =
  if elem (getFlagOrPrefix str) (long (flags option))
    then True
    else elem (getFlagOrPrefix str) (short (flags option))

getFlagOrPrefix :: String -> String
getFlagOrPrefix str
  | elem '=' str = ((takeWhile (/= '=') str))
  | otherwise = str

-- FILES -----------------------------------------------------------------
parseOptionFileName :: [String] -> (String, [String])
parseOptionFileName (x1:x2:xs)
  | length x1 == 1 = (x2, xs) --single letter
...
-- -- RANGE --------------------------------------------------------------
data GnuRange = GnuRange
  { min :: Integer
  , max :: Integer
  } deriving (Show)

-- -- TIME ---------------------------------------------------------------
data GnuTime = GnuTime
  { units :: Float
  , unitType :: TimeSuffix
  }
...
instance Read GnuTime where
  readsPrec _ (x) = do
    (num, xx) <- readsPrec 0 x :: [(Float, String)]
...
instance Read GnuSize where
  readsPrec _ (x) = do
    (pre, xx) <- readsPrec 0 x :: [(Prefix, String)]
