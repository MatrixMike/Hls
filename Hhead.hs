...
processArgs (x:xs) opts =
  case x of
    ...
    "-n" ->
      processArgs (drop 1 xs) opts {countLines = (read (head xs)) :: Integer}
    "-c" ->
      processArgs (drop 1 xs) opts {countBytes = (read (head xs)) :: Integer}
...
helpText :: [String]
helpText =
  [ "Usage: /home/bminerds/x/coreutils/src/head [OPTION]... [FILE]..."
...
versionText :: [String]
versionText =
  [ "Hhead (Haskell implementation of GNU head) 1.0"
